...
def analyze_dhcp_offer(network_interface, transaction_id):
    def dhcp_offer(packet):
        if packet.haslayer(scapy.DHCP) and packet[scapy.DHCP].options[0][1] == 2:
            ...
            send_dhcp_request(network_interface, server_ip, transaction_id)
            
    scapy.sniff(iface=network_interface, prn=dhcp_offer, filter="udp and (port 67 or port 68)", count=1)

def send_dhcp_request(network_interface, server_ip, transaction_id):
    dhcp_request = scapy.Ether(dst="ff:ff:ff:ff:ff:ff") / \
                   scapy.IP(src="0.0.0.0", dst="255.255.255.255") / \
                   scapy.UDP(dport=67, sport=68) / \
                   scapy.BOOTP(chaddr=scapy.get_if_hwaddr(network_interface), xid=transaction_id) / \
                   scapy.DHCP(options=[("message-type", "request"), ("server_id", server_ip), ("requested_addr", server_ip), "end"])
    scapy.sendp(dhcp_request, iface=network_interface, verbose=0)
    print(f"DHCP Request sent to {server_ip} for its own IP")
